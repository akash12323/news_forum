...
class EntertainmentAdapter(val user:List<EntertainmentArticlesItem>):
    RecyclerView.Adapter<EntertainmentAdapter.ItemVieHolder>(){

    var onItemClick:((user: EntertainmentArticlesItem)->Unit)? = null
    ...
    inner class ItemVieHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        fun bind(user: EntertainmentArticlesItem){
            itemView.apply {
