...
class SportsAdapter(val user:List<SportsArticlesItem>):
    RecyclerView.Adapter<SportsAdapter.ItemVieHolder>(){

    var onItemClick:((user: SportsArticlesItem)->Unit)? = null
    ...
    inner class ItemVieHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        fun bind(user: SportsArticlesItem){
            itemView.apply {
